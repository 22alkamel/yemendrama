...
export default function Podcasts() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const podcastsData: Content[] = Array.isArray(data.data)
          ? data.data.filter((c: Content) => c.type === "podcast")
          : [];
        setContents(podcastsData);

        // استخراج التصنيفات من البودكاست
        const allCategories = new Set<string>();
        podcastsData.forEach((item: Content) => {
  ...
  const filteredPodcasts = contents.filter(
    (item) =>
      selectedGenre === "الكل" ||
